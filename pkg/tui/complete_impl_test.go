...
func TestCompletionListRows(t *testing.T) {
	t.Parallel()
	...
	t.Run("packs_into_columns", func(t *testing.T) {
		l := &completionList{items: itemsOf("aa", "bb", "cc", "dd")}
	...
	t.Run("single_column_when_narrow", func(t *testing.T) {
		l := &completionList{items: itemsOf("aaaa", "bbbb")}
	...
	t.Run("overflow_counts_remainder", func(t *testing.T) {
		l := &completionList{items: itemsOf("aa", "bb", "cc", "dd", "ee", "ff")}
	...
	// detail text forces one candidate per row so the description stays readable.
	t.Run("detail_forces_one_per_row", func(t *testing.T) {
		l := &completionList{items: []Completion{
